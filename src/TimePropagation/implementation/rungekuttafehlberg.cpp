...
//------------------------------------------------------------------------------
RungeKuttaFehlberg::RungeKuttaFehlberg(Config *cfg):
    TimePropagation(cfg)
{
    try{
        epsilon = cfg->lookup("timeIntegration.rungeKuttaFehlberg.epsilon");
    } catch (const SettingNotFoundException &nfex) {
        cerr << "RungeKuttaFehlberg(Congif *cfg)"
...
bool RungeKuttaFehlberg::stepForward()
{
    ...
    h->computeNewElements(C + m1/4.0, t);
    ...
    h->computeNewElements(C + 3.0/32*m1 + 9.0/32*m2, t);

    k3 = -i*dt*slater->computeRightHandSide(A + 3.0/32*k1 + 9.0/32*k2);
    ...
    h->computeNewElements(C + 1932.0/2197*m1 - 7200.0/2197*m2 + 7296.0/2197*m3, t);

    k4 = -i*dt*slater->computeRightHandSide(A + 1932.0/2197*k1 - 7200.0/2197*k2 + 7296.0/2197*k3);
    ...
    h->computeNewElements(C + 439.0/216*m1 - 8.0*m2 + 3680.0/513*m3 - 845.0/4104*m4, t);

    k5 = -i*dt*slater->computeRightHandSide(A + 439.0/216*k1 - 8.0*k2 + 3680.0/513*k3 - 845.0/4104*k4);
    ...
    h->computeNewElements(C - 8.0/27*m1 + 2.0*m2 - 3544.0/2565*m3 + 1859.0/4104*m4 - 11.0/40*m5, t);

    k6 = -i*dt*slater->computeRightHandSide(A - 8.0/27*k1 + 2.0*k2 - 3544.0/2565*k3 + 1859.0/4104*k4 - 11.0/40*k5);

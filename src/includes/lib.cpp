...
//        break;
    default:
        cout << "calulating factorial. N = " << n << " Higher values should be tabulated." << endl;
...
void gauher(double *x, double *w, int n) {
    {
        ...
        for (i=0;i<m;i++) {
            ...
            if (its >= MAXIT){
                cerr << "Error: too many iterations in gauher" << endl;
                exit(0);
            }
            x[i]=z;
            x[n-1-i] = -z;
...
#define N2 10000
#define N3 1000000
int mapTwoParticleStates(int a, int b, int c, int d)
{
    return a + b*N1 + c*N2 + d*N3;
}
//------------------------------------------------------------------------------
